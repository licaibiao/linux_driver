...
static struct i2c_client *ks103_i2c_client = NULL;
...
static int ks103_measure(struct i2c_client *client,struct ks103_report_s *pf)
{
	...
	i2c_smbus_read_i2c_block_data(client,KS103_REG,2,buf);
	i2c_smbus_write_byte_data(client,KS103_REG,KS103_RANGE);

	pf->distance = (buf[0] << 8) + buf[1];

	//printk("==%s==%d (%x,%x)\r\n",__func__,pf->distance,buf[0],buf[1]);
...
static ssize_t ks103_read(struct file *filp, char __user *buf, size_t size, loff_t *ppos)
{
	struct ks103_report_s data;
	//printk("==%s==\r\n",__func__);
	ks103_measure(ks103_i2c_client,&data);
